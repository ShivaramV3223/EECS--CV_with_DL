...
def compute_distances_one_loop(x_train, x_test):
  ...
  for i in range(num_train):
    diff = (X_train[i] - X_test).pow(2)
    dists[i, :] = torch.sum(diff, dim = 1).sqrt()
...
def compute_distances_no_loops(x_train, x_test):
  ...
  X_tr_sqr = torch.sum( X_train.pow(2), dim = 1)
  X_tr_sqr = X_tr_sqr.reshape(num_train, 1)
  
  X_ts_sqr = torch.sum(X_test.pow(2), dim=1)
  X_ts_sqr = X_ts_sqr.reshape(num_test, 1)
  
  prd = X_train @ X_test.t()
  
  dists = (X_tr_sqr + X_ts_sqr.t()  - 2*prd).sqrt()
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
    a , idx = torch.topk(dists[:,i], k=k, dim=0, largest=False)
    t = y_train[idx]
    y_pred[i], v  = torch.mode(t)
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  X_tr_folds = x_train_folds
  y_tr_folds = y_train_folds
  for k in k_choices:
    k_to_accuracies[k] = []
    for i in range(num_folds):
      X_valid = X_tr_folds[i]
      y_valid = y_tr_folds[i]
      X_train = torch.cat([X_tr_folds[idx] for idx in range(num_folds) if idx != i])
      y_train = torch.cat([y_tr_folds[idx] for idx in range(num_folds) if idx != i])
      classifier = KnnClassifier(X_train, y_train)
      acc = classifier.check_accuracy(X_valid, y_valid, k= k, quiet= True)
