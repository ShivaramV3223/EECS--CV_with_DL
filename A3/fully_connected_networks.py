...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    self.params['W1'] = torch.normal(0.0, std= weight_scale, size=(input_dim, hidden_dim), dtype = dtype, device = device)
    self.params['b1'] = torch.zeros((hidden_dim,), dtype = dtype, device = device)
    self.params['W2'] = torch.normal(0.0, std = weight_scale, size = (hidden_dim, num_classes), dtype = dtype, device = device)
  ...
  def loss(self, X, y=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    W1 = self.params['W1'].clone()
    b1 = self.params['b1'].clone()
    W2 = self.params['W2'].clone()
    b2 = self.params['b2'].clone()
    
    l = Linear_ReLU()
    h1, cache1 = l.forward(X, W1, b1)
    scores, cache2 = l.forward(h1, W2, b2)
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    dh1, dW2, db2 = l.backward(dscores, cache2)
    dx, dW1, db1 = l.backward(dh1, cache1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in range(self.num_layers):
      self.params['W{}'.format(i+1)] = torch.normal(0.0, weight_scale, size = (dims[i], dims[i+1]), dtype = self.dtype).cuda()
      self.params['b{}'.format(i+1)] = torch.zeros(size = (dims[i+1], ), dtype = self.dtype).cuda()
  ...
  def loss(self, X, y=None):
    ...
    l = Linear_ReLU()
    caches = []
    h, cache = l.forward(X, self.params['W1'], self.params['b1'])
    caches.append(cache)
    for i in range(2, self.num_layers + 1):
      h_, cache = l.forward(h, self.params['W{}'.format(i)], self.params['b{}'.format(i)])
      h = h_
      caches.append(cache)
    ...
    for i in reversed(range(1, self.num_layers + 1)):
        dout_, dw, db = l.backward(dout, caches[i - 1])
        grads['W{}'.format(i)] = dw + 2* self.reg * self.params['W{}'.format(i)]
        grads['b{}'.format(i)] = db
        dout = dout_
        loss += self.reg*torch.sum(self.params['W{}'.format(i)] * self.params['W{}'.format(i)])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def create_solver_instance(data_dict, dtype, device):
  ...
  solver = None
  # Replace "pass" statement with your code
  solver = Solver(model,data_dict, device = device,num_epochs = 220)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return solver
...
def get_five_layer_network_params():
  ...
  # training accuracy within 20 epochs.                                      #
  ############################################################################
  learning_rate = 0.095  # Experiment with this!
...
def adam(w, dw, config=None):
  ...
  w += -config['learning_rate']*config['m']/(torch.sqrt(config['v']) + config['epsilon'])
  next_w = w
  #############################################################################
  #                              END OF YOUR CODE                             #
  #############################################################################
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      ###########################################################################
      # Replace "pass" statement with your code
      mask = ((torch.randn(*x.shape) < p)/p).cuda()
      out = x*mask
      ###########################################################################
      #                             END OF YOUR CODE                            #
      ###########################################################################
