...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    for n in range(N):
      for f in range(F):
        for h in range(H_):
          for wd in range(W_):
            hs, ws = h*stride, wd*stride
            he, we = hs +HH, ws + WW
            out[n,f,h,wd] = torch.sum(x[n,:,hs:he,ws:we] *w[f]) + b[f]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, w, b, conv_param)
  ...
  def backward(dout, cache):
    ...
    N, C, H, W = x.shape
    F, C, HH, WW = w.shape
    N_, F_, H_, W_ = dout.shape
    x_pad = torch.nn.functional.pad(x, (pad, pad, pad, pad))
    
    H_X_np = H - 2*pad
    W_X_np = W - 2*pad
    dw = torch.zeros(w.shape, dtype = x.dtype, device = x.device)
      ...
      for c in range(C):
        for h in range(HH):
          for wd in range(WW):
            ...
            dw[f, c, h, wd] += torch.sum(x[:,c,hs:he,ws:we]*dout[:,f,:, :])
      ...
      for f in range(F):
        for h in range(H_):
          for wd in  range(W_):
            ...
            dx[n, :, hs:he, ws:we] += dout[n, f, h, wd] * w[f]
    ...
    for n in range(N):
      for c in range(C):
        for h in range(H_):
          for wd in range(W_):
            hs, ws = h*stride, wd*stride
            he, we = hs + pool_height, ws + pool_width
            
            out[n, c, h, wd] = torch.max(x[n, c, hs:he, ws:we])
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, pool_param)
      ...
      for c in range(C):
        for h in range(H_):
          for wd in  range(W_):
            ...
            x_slice = x[n, c, hs:he, ws:we]
            max_ = torch.max(x_slice)
            dx[x == max_] += dout[n,c,h,wd]
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    self.params['W1'] = torch.normal(0.0, weight_scale, (F, C, HH, WW), dtype = self.dtype, device = device)
    self.params['b1'] = torch.zeros((num_filters,), dtype = self.dtype, device = device)
    self.params['W2'] = torch.normal(0.0, weight_scale, (F*H_*W_,hidden_dim), dtype = self.dtype, device = device)
    self.params['b2'] = torch.zeros((hidden_dim,), dtype = self.dtype,  device = device)
    self.params['W3'] = torch.normal(0.0, weight_scale, (hidden_dim, num_classes), dtype = self.dtype, device = device)
  ...
  def loss(self, X, y=None):
    ...
    # Replace "pass" statement with your code
    caches = []
    h1, cache = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
    caches.append(cache)
  
    h2, cache = Linear_ReLU.forward(h1, W2, b2)
    caches.append(cache)
    ...
    dout, dw3, db3 = Linear.backward(dout, caches[2])
    dout, dw2, db2 = Linear_ReLU.backward(dout, caches[1])
    dout, dw1, db1 = Conv_ReLU_Pool.backward(dout, caches[0])
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    for i in range(len(num_filters)):
      ...
      if weight_scale == 'kaiming':
        self.params['W{}'.format(i+1)] = kaiming_initializer(Din=C, Dout=F, K=3,relu = True, dtype = self.dtype, device = device)
      else:
        self.params['W{}'.format(i+1)] = torch.normal(0.0, weight_scale, (F, C, 3, 3), dtype = self.dtype, device = device)
      
      if batchnorm:
        self.params['gamma{}'.format(i+1)] = torch.ones((F,), dtype = self.dtype, device = device)
        self.params['beta{}'.format(i+1)] = torch.zeros((F,), dtype = self.dtype, device = device)
      
      self.params['b{}'.format(i+1)] = torch.zeros((F,), dtype = self.dtype, device = device)
      C = F
    
    H_ = int(H/(2 ** len(max_pools)))
    W_ = int(W/(2**len(max_pools)))
    if weight_scale == 'kaiming':
      self.params['W{}'.format(self.num_layers)] = kaiming_initializer(Din = C*H_*W_, Dout=num_classes, relu = False, dtype = self.dtype, device = device)
    else:
      self.params['W{}'.format(self.num_layers)] = torch.normal(0.0, weight_scale, (C*H_*W_, num_classes), dtype = self.dtype, device = device)
    self.params['b{}'.format(self.num_layers)] = torch.zeros((num_classes,), dtype = self.dtype, device = device)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1):
      if i in self.max_pools:
        if self.batchnorm:
          h, cache = Conv_BatchNorm_ReLU_Pool.forward(h, self.params['W{}'.format(i+1)], self.params['b{}'.format(i+1)], self.params['gamma{}'.format(i+1)], self.params['beta{}'.format(i+1)], conv_param, bn_param, pool_param)
        else:
          h, cache = Conv_ReLU_Pool.forward(h, self.params['W{}'.format(i+1)], self.params['b{}'.format(i+1)], conv_param, pool_param)
      else:
        if self.batchnorm:
          h, cache = Conv_BatchNorm_ReLU.forward(h, self.params['W{}'.format(i+1)], self.params['b{}'.format(i+1)], self.params['gamma{}'.format(i+1)], self.params['beta{}'.format(i+1)], conv_param, bn_param)
        else:
          h, cache = Conv_ReLU.forward(h, self.params['W{}'.format(i+1)], self.params['b{}'.format(i+1)], conv_param)
        
      caches.append(cache)
      
    scores, cache = Linear.forward(h, self.params['W{}'.format(self.num_layers)], self.params['b{}'.format(self.num_layers)])
    ...
    # Replace "pass" statement with your code
    loss, dout = softmax_loss(scores, y)
    dout, grads['W{}'.format(self.num_layers)], grads['b{}'.format(self.num_layers)] = Linear.backward(dout, cache)

    loss += self.reg*torch.sum(self.params['W{}'.format(self.num_layers)]*self.params['W{}'.format(self.num_layers)])
    ...
    for i in  reversed(range(len(caches))):
      ...
      if i in self.max_pools:
        if self.batchnorm:
          dout, grads['W{}'.format(i+1)], grads['b{}'.format(i+1)], grads['gamma{}'.format(i+1)], grads['beta{}'.format(i+1)] = Conv_BatchNorm_ReLU_Pool.backward(dout, caches[i])
        else:
          dout, grads['W{}'.format(i+1)], grads['b{}'.format(i+1)] = Conv_ReLU_Pool.backward(dout, caches[i])
      
      else:
        if self.batchnorm:
          dout, grads['W{}'.format(i+1)], grads['b{}'.format(i+1)], grads['gamma{}'.format(i+1)], grads['beta{}'.format(i+1)] = Conv_BatchNorm_ReLU.backward(dout, caches[i])
        else:
          dout, grads['W{}'.format(i+1)], grads['b{}'.format(i+1)] = Conv_ReLU.backward(dout, caches[i])

      loss += self.reg*torch.sum(self.params['W{}'.format(i+1)] * self.params['W{}'.format(i+1)])
      grads['W{}'.format(i+1)] += 2*self.reg*self.params['W{}'.format(i+1)]
      
      if self.batchnorm:
        loss += self.reg*torch.sum(self.params['gamma{}'.format(i+1)] ** 2)
        grads['gamma{}'.format(i+1)] += 2*self.reg*self.params['gamma{}'.format(i+1)]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
  ...
  model = DeepConvNet(input_dims = input_dims, num_classes = 10,
                     ...
                     max_pools = [0, 1, 2, 3, 4],
                     reg = 1e-2, weight_scale = 'kaiming', dtype = dtype, device = device)
...
class BatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
      ...
      std_1 = 1./std
      x_norm = x_centered*std_1
      out = gamma*x_norm + beta
      cache = (x_centered,  x_norm, std_1, std, var, eps, mean,gamma, bn_param)
      #######################################################################
      #                           END OF YOUR CODE                          #
      #######################################################################
    elif mode == 'test':
  ...
  def backward(dout, cache):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    x_centered, x_norm, std_1, std, var, eps, mean, gamma, bn_param = cache
    ...
    if mode == 'train':
      ...
      dgamma = torch.sum(dout * x_norm, axis = 0)

      dx_norm = dout * gamma
      dx_centered = dx_norm * std_1
      dstd_1 = torch.sum(x_centered * dx_norm, axis = 0)
      dstd = -1. / (std ** 2) * dstd_1
      dvar = 0.5 * 1./torch.sqrt(var + eps) * dstd
      dsq = 1./N * torch.ones((N,D), dtype = dout.dtype, device = dout.device)*dvar
      dx_centered += 2 * x_centered * dsq
      dx = dx_centered
      dmean = -1 * torch.sum(dx_centered, axis = 0)
      dx += 1./N * torch.ones((N,D), dtype = dout.dtype, device = dout.device) * dmean
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
  ...
  def backward_alt(dout, cache):
    ...
    if mode == 'train':
      ...
      dx = (gamma * std_1) * (N * dout - x_norm*dgamma - dbeta)
...
class SpatialBatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    # Replace "pass" statement with your code
    N, C, H, W = x.shape
    x_ = x.permute(1, 0, 2, 3).reshape(C, -1).T
    x_, cache = BatchNorm.forward(x_, gamma, beta, bn_param)
    out = x_.T.reshape(C, N, H, W).permute(1, 0, 2, 3)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
  ...
  def backward(dout, cache):
    ...
    # Replace "pass" statement with your code
    N, C, H, W = dout.shape
    dout_ = dout.permute(1, 0, 2, 3).reshape(C, -1).T
    dx_, dgamma, dbeta = BatchNorm.backward(dout_, cache)
    dx= dx_.T.reshape(C, N, H, W).permute(1, 0, 2, 3)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
