...
def sample_batch(X, y, num_train, batch_size):
  ...
  #########################################################################
  # Replace "pass" statement with your code
  idxs = torch.randint(0, num_train, (batch_size, 1))
  X_batch = X[idxs, :].reshape(batch_size, X.shape[1])
...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1e-3, 1e-6, 1e-4, 1e-2]
  regularization_strengths = [1e-5, 1e-4, 1e-2, 1e-3]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  loss_history = cls.train(data_dict['X_train'], data_dict['y_train'], learning_rate = lr,
                              reg = reg, num_iters = num_iters, batch_size = 300, verbose = False)
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(N):
    ...
    loss += -correct_class_score + torch.log(torch.sum(torch.exp(scores)))
    for j in range(C):
      if j == y[i]:
        dW[:, y[i]] += (torch.exp(scores[y[i]])/torch.sum(torch.exp(scores)) - 1) * X[i]
      else:
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  summ = torch.sum(exps, dim=1)
  loss += torch.sum(-correct_class_scores + torch.log(summ))
  loss /= N
  loss += reg*torch.sum(W*W)

  dW += X.t() @ (exps/summ.reshape(-1,1))
  tmp = torch.zeros_like(scores)
  tmp[range(N), y] -= 1
  dW += X.t() @ tmp
  dW /= N
  dW += 2*reg*W
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1e2, 1e-3, 1e-2, 1e-4]
  regularization_strengths = [1e3, 1e-2, 1e-6, 1e-5]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
