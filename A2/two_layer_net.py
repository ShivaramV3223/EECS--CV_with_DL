...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    sum_exps = torch.sum(exps, dim=1)
    correct_class_scores = scores[range(N), y]
    loss = torch.sum(-correct_class_scores + torch.log(sum_exps))
    loss /= N
    ...
    db2 += torch.sum(ds, dim = 0)
    grads['b2'] = db2
    
    dh1 += ds @ W2.t()

    tmp = torch.zeros_like(z1)
    tmp[z1 > 0] = 1
    dz1 += dh1 * tmp
...
def nn_get_search_params():
  ...
  regularization_strengths = [1e0, 1e-2, 1e-4]
  learning_rate_decays = [1.0, 0.90, 0.95]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  lrs, hss, regs, lrds = get_param_set_fn()
  for lr in lrs:
    for hs in hss:
      for reg in regs:
        for lrd in lrds:
