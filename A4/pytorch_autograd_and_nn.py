...
def three_layer_convnet(x, params):
  ...
  x = F.relu(F.conv2d(x, conv_w1, conv_b1, padding = (2, 2)))
  x = F.relu(F.conv2d(x, conv_w2, conv_b2, padding = (1, 1)))
  x = flatten(x)
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  H_1 = int(1 + (H - kernel_size_1 + 2*2)/1)
  W_1 = int(1 + (W - kernel_size_1 + 2*2)/1)
  
  conv_w2 = torch.empty(channel_2, channel_1, kernel_size_2, kernel_size_2)
  H_2 = int(1 + (H_1 - kernel_size_2 + 2*1)/1)
  W_2 = int(1 + (W_1 - kernel_size_2 + 2*1)/1)

  fc_w = torch.empty(num_classes, channel_2*H_2*W_2)
 

  conv_w1 = torch.nn.init.kaiming_normal_(conv_w1, nonlinearity = 'relu').cuda()
  conv_w1.requires_grad = True
  conv_b1 = torch.zeros((channel_1,), dtype = dtype, device = device)
  conv_b1.requires_grad = True

  conv_w2 = torch.nn.init.kaiming_normal_(conv_w2, nonlinearity = 'relu').cuda()
  conv_w2.requires_grad = True
  conv_b2 = torch.zeros((channel_2,), dtype = dtype, device = device)
  conv_b2.requires_grad = True

  fc_w = torch.nn.init.kaiming_normal_(fc_w, nonlinearity='relu').cuda()
...
class ThreeLayerConvNet(nn.Module):
  ...
  #   1. Convolutional layer with channel_1 5x5 filters with zero-padding of 2  
    ...
    ############################################################################
    # Replace "pass" statement with your code
    self.conv_1 = nn.Conv2d(in_channel, channel_1, 5,  padding = (2,2))
    self.conv_2 = nn.Conv2d(channel_1, channel_2, 3,  padding = (1,1))
...
def initialize_three_layer_conv_part4():
  ...
  model = nn.Sequential(OrderedDict([
    ('conv_1', nn.Conv2d(C, channel_1, kernel_size_1, padding = (pad_size_1,pad_size_1))),
    ('relu1', nn.ReLU()),
    ('conv_2', nn.Conv2d(channel_1, channel_2, kernel_size_2, padding = (pad_size_2,pad_size_2))),
    ('relu2', nn.ReLU()),
    ('flatten', Flatten()),
    ('fc', nn.Linear(channel_2*H_2*W_2, num_classes)),
  ]))
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.net = nn.Sequential(
      nn.BatchNorm2d(Cin),
      nn.ReLU(),
      nn.Conv2d(Cin,Cout, 3,padding = (1,1),stride = std),
      nn.BatchNorm2d(Cout),
      nn.ReLU(),
      nn.Conv2d(Cout,Cout, 3,padding = (1,1))
    )
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    self.block = nn.Sequential(
      nn.BatchNorm2d(Cin),
      nn.ReLU(),
      nn.Conv2d(Cin, Cout//4, 1, stride = std),
      nn.BatchNorm2d(Cout//4),
      nn.ReLU(),
      nn.Conv2d(Cout//4, Cout//4, 3, padding = (1,1)),
      nn.BatchNorm2d(Cout//4),
      nn.ReLU(),
