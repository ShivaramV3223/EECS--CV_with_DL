...
def compute_saliency_maps(X, y, model):
  ...
  torch.sum(correct_class_score).backward()
  saliency,idxs = X.grad.max(dim = 1)
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  for i in range(max_iter):
    ...
    with torch.no_grad():
      grad = X_adv.grad.data * learning_rate/(torch.sum(X_adv.grad.data)**2)
      X_adv += grad
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    scores = model(img)
    correct_class_score = scores[:,target_y] - l2_reg*torch.sum(img*img)
    correct_class_score.backward()
